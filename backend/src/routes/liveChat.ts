...
async function getAutomatedResponse(message: string, pool: Pool, userId?: string): Promise<{ response: string | null, canHandle: boolean }> {
  ...
  app.post('/api/live-chat/support-request', async (req, res) => {
    try {
      ...
      if (!sessionId || !subject || !description) {
        return sendError(res, 400, 'sessionId, subject, and description are required')
      }
      ...
      return sendError(res, 500, 'Failed to create support request', err)
    }
  })
}
